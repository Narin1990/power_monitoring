...
// Robust UART2 line buffer (newline-delimited JSON)
...
void setDefaults() {
  ...
  strlcpy(cfg.wifi_pass, "", sizeof(cfg.wifi_pass));

  strlcpy(cfg.mqtt_host, "", sizeof(cfg.mqtt_host));
  cfg.mqtt_port = 1883;
  strlcpy(cfg.mqtt_user, "", sizeof(cfg.mqtt_user));
  strlcpy(cfg.mqtt_pass, "", sizeof(cfg.mqtt_pass));
  strlcpy(cfg.mqtt_topic, "power_detector", sizeof(cfg.mqtt_topic));
  ...
  cfg.publish_ms = 10000;  // default 10 seconds (normal mode)

  // power saving defaults
  cfg.sleep_enable = false;
  cfg.sleep_s = 60;
  cfg.sample_gap_ms = 250;
}
...
void publishPowerDetectorSleep(float avgA, float maxA, float batteryV) {
  ...
  JsonObject pd = doc.createNestedObject("power_detector");
  pd["current_avg(A)"] = avgA;
  pd["current_max(A)"] = maxA;
  pd["battery"] = batteryV;
  pd["mac"] = getBaseMacHex();
...
// ================= UART2 stream =================
void sendDataFrameUart2(float currentA, float batteryV) {
  DynamicJsonDocument out(256);
  out["type"] = "data";
  out["irms"] = currentA;
  out["vbat"] = batteryV;
...
// ✅ Robust UART2 receiver: ignore junk before '{', cut after last '}', drop overflow
void pollCfgUart() {
  while (CFG.available()) {
...
void setup() {
  ...
  CFG.begin(CFG_BAUD, SERIAL_8N1, CFG_RX, CFG_TX);
  CFG.setRxBufferSize(2048);
  delay(120);
  ...
  Serial.println("Boot: UART0=LOG, UART2=CFG");
  ...
  if (cfg.sleep_enable) {
    ...
    bool mok = false;
    if (wok && strlen(cfg.mqtt_host) > 0) mok = mqttConnect(10000);

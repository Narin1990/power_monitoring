...
#define WIFI_RESTART_TIMEOUT_MS   60000UL
#define MQTT_RESTART_TIMEOUT_MS   60000UL
#define PUBLISH_RESTART_TIMEOUT_MS 900000UL
#define DAILY_RESTART_MS          86400000UL
...
RTC_DATA_ATTR uint32_t rtcSleepCycles = 0;
uint32_t bootMs = 0;
uint32_t lastPublishOkMs = 0;

// ================= Forward decl =================
float readIrmsA_50Hz(uint16_t window_ms);
float readVbat(uint16_t samples = 16);
bool wifiConnect(uint32_t timeoutMs = 20000, bool cleanStart = true);
...
void setDefaults() {
  ...
  */
  ...
  // Publish interval
  cfg.publish_ms = 120000;  // default 2 minutes

  // Power saving defaults
  cfg.sleep_enable = true;
  cfg.sleep_s = 60;       // 2 minutes
...
bool wifiConnect(uint32_t timeoutMs, bool cleanStart) {
  ...
  WiFi.setHostname("power-detector");

  uint32_t totalStart = millis();
  uint8_t attempt = 0;

  while ((millis() - totalStart) < timeoutMs) {
    attempt++;

    if (cleanStart || attempt > 1) {
    ...
    uint32_t attemptBudget = 8000;
    uint32_t remaining = timeoutMs - (millis() - totalStart);
    if (remaining < attemptBudget) attemptBudget = remaining;

    while ((millis() - attemptStart) < attemptBudget) {
...
float readIrmsA_50Hz(uint16_t window_ms) {
  ...
  float currentA = rms_mv * cfg.ct_cal;
  currentA = currentA - 0.55f;
...
void publishPowerDetector(float currentA, float batteryV) {
  ...
  bool ok = mqtt.publish(cfg.mqtt_topic, payload.c_str());
  if (ok) lastPublishOkMs = millis();
}
...
void checkNetworkWatchdogs() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if ((now - wifiDownSince) >= WIFI_RESTART_TIMEOUT_MS) {
      restartNow("wifi_timeout");
    }
  } else {
    wifiDownSince = 0;
  }
  ...
  if (WiFi.status() == WL_CONNECTED && strlen(cfg.mqtt_host) > 0) {
    if (!mqtt.connected()) {
      ...
      if ((now - mqttDownSince) >= MQTT_RESTART_TIMEOUT_MS) {
        restartNow("mqtt_timeout");
      }
    } else {
      mqttDownSince = 0;
    }
  ...
  if (cfg.publish_ms > 0) {
    uint32_t publishTimeout = PUBLISH_RESTART_TIMEOUT_MS;
    uint32_t dynamicTimeout = cfg.publish_ms * 6UL;
    if (dynamicTimeout > publishTimeout) publishTimeout = dynamicTimeout;

    if ((now - lastPublishOkMs) >= publishTimeout) {
      restartNow("publish_timeout");
    }
  }

  if ((now - bootMs) >= DAILY_RESTART_MS) {
    restartNow("daily_restart");
  }
}
...
void handleCommandLine(const char *line) {
  ...
  if (strcmp(cmd, "test_wifi") == 0) {
    bool ok = (WiFi.status() == WL_CONNECTED) ? true : wifiConnect(20000, true);
    ...
    out["wifi_status"] = wifiStatusText(WiFi.status());
    out["mac"] = getWiFiMacString();
    cfgSendJson(out);
    return;
  }
  ...
  if (strcmp(cmd, "test_mqtt") == 0) {
    bool w = (WiFi.status() == WL_CONNECTED) ? true : wifiConnect(15000, true);
    bool ok = false;
    ...
    out["sleep_s"] = cfg.sleep_s;
    out["sample_gap_ms"] = cfg.sample_gap_ms;
    out["rtc_sleep_cycles"] = rtcSleepCycles;
    out["base_mac"] = getBaseMacHex();
    out["mac"] = getWiFiMacString();
    cfgSendJson(out);
    return;
...
void setup() {
  ...
  if (wakeCause == ESP_SLEEP_WAKEUP_TIMER) rtcSleepCycles++;
  else rtcSleepCycles = 0;
  ...
  hello["rtc_sleep_cycles"] = rtcSleepCycles;
  ...
  if (cfg.sleep_enable) {
    ...
    uint32_t cyclesPerDay = 86400UL / (cfg.sleep_s ? cfg.sleep_s : 1);
    if (cyclesPerDay < 1) cyclesPerDay = 1;
    if (rtcSleepCycles >= cyclesPerDay) {
      rtcSleepCycles = 0;
      restartNow("daily_restart_sleep_mode");
    }

    psStep("wifi_connecting");

    bool wok = (WiFi.status() == WL_CONNECTED) ? true : wifiConnect(20000, true);
  ...
  if (strlen(cfg.wifi_ssid) > 0) {
    ...
    bool wok = wifiConnect(20000, true);
...
void loop() {
  ...
  if (WiFi.status() != WL_CONNECTED && strlen(cfg.wifi_ssid) > 0) {
    if (millis() - lastWiFiTry > 5000) {
      ...
      bool ok = wifiConnect(20000, false);
      Serial.printf("[NET] WiFi reconnect result=%s status=%s\n", ok ? "OK" : "FAIL", wifiStatusText(WiFi.status()));
    }
  }
